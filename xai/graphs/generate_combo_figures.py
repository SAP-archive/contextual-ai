...
def visualize_feature_for_similar_classes(dataset, feature_name, base_class, similar_class, sub_confusion_matrix):
    ...
    image_path_a = os.path.join(constants.FIGURE_PATH,
                                ('%s_%s_%s.png' % (dataset, feature_name, base_class)).replace('/', '-'))
    if not os.path.exists(image_path_a):
        image_path_a = os.path.join(constants.FIGURE_PATH,
                                    ('%s_%s_%s.png' % (constants.KEY_DATA_ALL, feature_name, base_class)).replace('/',
                                                                                                                  '-'))
    image_path_b = os.path.join(constants.FIGURE_PATH,
                                ('%s_%s_%s.png' % (dataset, feature_name, similar_class)).replace('/', '-'))
