...
class ModelInterpreter:
    ...
    def interpret_model(self, samples: List[numpy.ndarray], stats_type: str = 'top_k', k: int = 5):
        ...
        """
        if self._explainer:
            _explainer_aggregator = ExplanationAggregator()

            for sample in samples:
                exp = self._explainer.explain_instance(instance=sample)
                _explainer_aggregator.feed(explanation=exp)

            _explainer_aggregator.get_statistics(stats_type=stats_type, k=k)
        else:
    ...
    def error_analysis(self, class_num: int, valid_x: List[numpy.ndarray], valid_y: Union[List[int], List[str]],
        ...
        """
        error_analysis_dict = defaultdict(ExplanationAggregator)

        for sample, gt_label in zip(valid_x, valid_y):
