...
def convert_response_json_to_training_meta(response_json_file, meta_json_file, output_folder):
    ...
    for idx, result in enumerate(results):
        ...
        training_result[constants.KEY_DATA_TEST] = defaultdict(dict)
        for metric, metric_value in result.items():

            if metric == RESPONSE_KEY_CM:
                training_result[constants.KEY_DATA_TEST][constants.TRAIN_TEST_CM] = metric_value
            elif RESPONSE_PREFIX_AVE in metric:
                _true_metric = metric.replace(RESPONSE_PREFIX_AVE, "")
                training_result[constants.KEY_DATA_TEST][_true_metric]["average"] = metric_value
            else:
                training_result[constants.KEY_DATA_TEST][metric]["class"] = dict()
                assert (len(labels) == len(metric_value))
                label_metric = zip(labels, metric_value)
                for l, v in label_metric:
                    training_result[constants.KEY_DATA_TEST][metric]["class"][l] = v

        new_meta_json = deepcopy(meta_json)
        new_meta_json[constants.METADATA_KEY_DATA_SEC][constants.META_KEY_ATTRIBUTE_FEATURE][label_key][
            constants.META_KEY_FIELD_TYPE] = constants.FEATURE_DATA_TYPE_LABEL[0]

        with open(os.path.join(output_folder, 'meta_%s.json' % idx), 'w') as f:
            json.dump(new_meta_json, f)
        with open(os.path.join(output_folder, 'eval_%s.json' % idx), 'w') as f:
...
def generate_seperate_reports(data_path):
    ...
    report_num = convert_response_json_to_training_meta(response_json_file, meta_json_file, temp_path)

    for idx in range(report_num):
        ## generate seperate folder and prepare the file for generate report
        rep_output_path = os.path.join(output_path, 'report_%s' % idx)
        rep_data_path = os.path.join(temp_path, 'report_%s' % idx)
        if not os.path.exists(rep_output_path):
            os.mkdir(rep_output_path)
        if not os.path.exists(rep_data_path):
            os.mkdir(rep_data_path)

        shutil.copyfile(os.path.join(temp_path, 'meta_%s.json' % idx),
        ...
        with open(os.path.join(temp_path, 'eval_%s.json' % idx), 'r') as f:
            eval_result = json.load(f)
        with open(os.path.join(temp_path, 'meta_%s.json' % idx), 'r') as f:
            meta = json.load(f)
